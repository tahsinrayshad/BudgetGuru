...
import { getSpendingDistribution } from "@/services/analytics.service";
...
export async function GET(request: NextRequest): Promise<NextResponse> {
  try {
    ...
    const result = await getSpendingDistribution(tokenResponse.user.userId, period);
    ...
    return NextResponse.json(
      {
        success: false,
        message:
          error instanceof Error ? error.message : "Failed to fetch spending distribution",
      },
      { status: 500 }
    );
  }
