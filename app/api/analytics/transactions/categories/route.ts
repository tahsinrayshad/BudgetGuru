...
import { getCategoryWiseBreakdown } from "@/services/analytics.service";
...
export async function GET(request: NextRequest): Promise<NextResponse> {
  try {
    ...
    const result = await getCategoryWiseBreakdown(tokenResponse.user.userId, period);
    ...
    return NextResponse.json(
      {
        success: false,
        message:
          error instanceof Error ? error.message : "Failed to fetch category-wise breakdown",
      },
      { status: 500 }
    );
  }
